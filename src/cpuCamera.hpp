	...
	public:
		...
		void preliminary_snap(shared_ptr<Viewport> depthMap,
			...
			for (uint32_t i = 0; i < _numThreads - 1; ++i) {
				threads.push_back(std::thread(&cpuCamera::preliminary_subSnap, this, cells_per_thread*i, cells_per_thread, depthMap, normalMap, colourMap, std::ref(cells), shapes, lights, std::ref(count4), false));
			}
		...
		void final_snap(shared_ptr<Viewport> viewport, Shapes shapes, Lights lights) {
			...
			for (uint32_t i = 0; i < _numThreads - 1; ++i) {
				threads.push_back(std::thread(&cpuCamera::final_subSnap, this, cells_per_thread*i, cells_per_thread, viewport, std::ref(cells), shapes, lights, std::ref(count4), false));
			}
